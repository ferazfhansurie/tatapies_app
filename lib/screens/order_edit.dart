...
class OrderEdit extends StatefulWidget {
  ...
  int quantity;
  String orderId;
  OrderEdit({Key? key, required this.orderId, required this.companyId,required this.name,required this.address,required this.remarks,required this.delivery,required this.total,required this.pie,required this.size,required this.quantity}) : super(key: key);
...
class _OrderEditState extends State<OrderEdit> {
  ...
  void _addNewPieOrder2(String pie,String size,int quantity) {
    setState(() {
      pieOrders.add(PieOrder(type:pie, size: size, quantity: quantity));
    });
  }
  ...
  deliveryFeeController.text=widget.delivery;
  totalController.text = widget.total;
      _addNewPieOrder2(widget.pie,widget.size,widget.quantity); // Start with one pie order
  }
...
void saveOrder() async {
  ...
  for (var pieOrder in pieOrders) {
    ...
    try {
      // Check if the pieOrder has an orderId
      if (widget.orderId != null) {
        await FirebaseFirestore.instance
            .collection("companies")
            .doc(widget.companyId)
            .collection("orders")
            .doc(widget.orderId) // Use orderId to reference the document
            .update(orderData); // Update the existing document

        // Optionally inform user of success
      } else {
        // Handle the case where orderId is null or does not exist
        // Decide whether to create a new order or perform some other action
      }
    } catch (e) {
      // Handle errors
      print(e);
  ...
  Toast.show(context,"success","Order Updated"); // Update the message accordingly
}
}
